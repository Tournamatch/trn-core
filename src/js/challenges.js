            ...
            link.addEventListener('trn.confirmed.action', function (event) {
                ...
                xhr.onload = function () {
                    if (xhr.status === 204) {
                        ...
                        document.getElementById('trn-delete-challenge-response').innerHTML = `<div class="alert alert-danger"><strong>${options.language.failure}</strong>: ${response.message}</div>`;
                    }
                };
