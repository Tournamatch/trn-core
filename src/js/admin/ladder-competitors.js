...
(function ($) {
    ...
    window.addEventListener('load', function () {
        let options = trn_ladder_competitors_options;
                    ...
                    xhr.onload = function () {
                        ...
                        if (xhr.status === 200) {
                            ...
                            if (players.length > 0) {
                                ...
                                document.getElementById('trn-ladder-competitors-response').innerHTML = `<p class="notice notice-error"><strong>${options.language.failure}:</strong> ${options.language.no_competitor}</p>`;
                            }
                        } else {
                            reject();
                        }
        ...
        function registerCompetitor(competitionId, competitorId, competitorType) {
            ...
            xhr.onload = function () {
                console.log(xhr);
                if (xhr.status === 201) {
                    document.getElementById('trn-ladder-competitors-response').innerHTML = `<p class="notice notice-success"><strong>${options.language.success}:</strong> ${options.language.success_message}</p>`;
                    document.getElementById('trn-ladder-competitors-form').reset();
                } else {
                    document.getElementById('trn-ladder-competitors-response').innerHTML = `<p class="notice notice-error"><strong>${options.language.failure}:</strong> ${JSON.parse(xhr.response).message}</p>`;
                }
            };
